...
from lucid_kietzmannlab.optvis import param


def jitter(d, seed=None):
    def inner(t_image):
        t_image = tf.convert_to_tensor(t_image)
...
def random_scale(scales, seed=None):
    def inner(t):
        ...
        return tf.image.resize(t, scale_shape)
...
def _rand_select(xs, seed=None):
    rand_n = tf.random.uniform((), 0, len(xs), "int32", seed=seed)
