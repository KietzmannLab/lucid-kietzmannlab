...
from lucid_kietzmannlab.misc.io.reading import read
from lucid_kietzmannlab.misc.io.saving import ClarityJSONEncoder
...
def load_graphdef(model_url, reset_device=True):
    ...
    graph_def = tf.compat.v1.GraphDef.FromString(graphdef_string)
    if reset_device:
...
class GraphDefHelper:
    ...
    def __init__(self, graph_def):
        ...
        for node in graph_def.node:
            ...
            assert (
                node.name not in self.by_name
            )  # names should be unique I guess?
...
def frozen_default_graph_def(input_node_names, output_node_names):
    ...
    if len(input_graph_def.node) == 0:
        raise RuntimeError(
            "Default graph is empty. Is it possible your model wasn't constructed or is in a different graph?"
        )

    pruned_graph = tf.graph_util.remove_training_nodes(
        input_graph_def, protected_nodes=(output_node_names + input_node_names)
    )
    pruned_graph = tf.graph_util.extract_sub_graph(
        pruned_graph, output_node_names
    )
...
def infuse_metadata(graph_def, info):
    ...
    with temp_graph.as_default():
        tf.constant(
            json.dumps(info, cls=ClarityJSONEncoder), name=metadata_node_name
        )
