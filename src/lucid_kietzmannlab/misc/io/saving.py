...
from lucid_kietzmannlab.misc.io.serialize_array import _normalize_array
from lucid_kietzmannlab.misc.io.writing import write, write_handle
...
def save_npz(object, url):
    ...
    if type(object) is dict:
        ...
        log.warn(
            "Saving non dict or list as npz file, did you maybe want npy?"
        )
        np.savez(url, object)
...
def save_img(object, url, **kwargs):
    ...
    with write_handle(url) as handle:
        image.save(
            handle, **kwargs
        )  # will infer format from handle's url ext.
